...
**Known gap:** three short process videos embedded in the Spaceabet page (plus one
each on Y Conference and Generali Travel Insurance) are uploaded Figma video assets,
not served from the same static `/_assets/v11/<hash>.<ext>` path as images — fetching
them 404s. `download_images.py` skips them and falls back to nothing (Spaceabet,
whose gallery has other images) or the poster frame where one downloaded. Tracked as
a follow-up, not blocking the migration.
...
- Three process videos not yet portable (see "Known gap" above).
